...
describe('API', { concurrency: false }, () => {
  ...
  it('pull-tokens + public/channels: Bearer đối tác đi được, lạ thì 401', async () => {
    ...
    try {
      ...
      // Opt-in: tích published cho demo4 (meta, không restart) rồi mới lên danh mục.
      r = await authed('/api/sources/API1', {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          channels: [
      ...
      assert.ok(!list.channels.some((c) => c.name === 'demo5'), 'chưa tích thì không lên danh mục');
    } finally {
  ...
  it('epg mapping: id sai/trùng → 400, đúng → 201', async () => {
    ...
    r = await post('EPGMAP', [{ name: 'kenhEpg', serviceId: 61, isLive: false, partnerChannelId: 809, published: true }]);
    assert.equal(r.status, 201);
